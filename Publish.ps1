...
$SolutionOrProjectPath = "C:\Projects\JSONPrettyPrint\JSONPrettyPrint.sln"            # Solution or Project file you want to publish
$PublishPath = "C:\Projects\JSONPrettyPrint\bin\Release\net5.0\publish"          # The directory you want the published files to go in
$BinaryName = "JSONPrettyPrint"                                                  # Binary name from build, without the .exe extension
$PublishSingleFile = "true"                                                 # If true, produces a single file
$PublishTrimmed = "true"                                                   # If true, reduces the size of the assembly
$TrimMode = "Link"                                                              # Link or CopyUsed https://devblogs.microsoft.com/dotnet/app-trimming-in-net-5/
...
function Publish {
    ...
    # Publish
    dotnet publish -r $Runtime -c $DebugOrRelease -p:PublishSingleFile=$PublishSingleFile -p:PublishTrimmed=$PublishTrimmed -p:TrimMode=$TrimMode -p:IncludeNativeLibrariesForSelfExtract=$IncludeNativeLibrariesForSelfExtract -p:PublishReadyToRun=$PublishReadyToRun --nologo --output $PublishPath $SolutionOrProjectPath
